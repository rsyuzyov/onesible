...
Функция ЕстьФайлВыгрузкиНаКлиенте(Агент)
	...
	Если РегистрыСведений.СвойстваКомпьютеров.ПолучитьСвойство(Агент.Компьютер,, "ЕстьФайлВыгрузки") = "Да" Тогда
		Возврат Истина;
	КонецЕсли;
	
	Файл = Новый Файл(СтрЗаменить("\\192.168.20.18\public\ex\Message_ЦБ_М00.zip", "М00", СокрЛП(Агент.Код)));
	Если Файл.Существует() Тогда
		РегистрыСведений.СвойстваКомпьютеров.ЗаписатьСвойство(Агент.Компьютер,, "ЕстьФайлВыгрузки", Истина);
		Возврат Истина;
	КонецЕсли;
...
Функция КлиентОтчиталсяОПолученииФайлаВыгрузки(Агент)
	...
	Если РегистрыСведений.СвойстваКомпьютеров.ПолучитьСвойство(Агент.Компьютер,, "КлиентОтчиталсяОПолученииФайлаВыгрузки") = "Да" Тогда
		Возврат Истина;
	КонецЕсли;
	
	Файл = Новый Файл(СтрЗаменить("\\192.168.20.18\public\ex\Message_ЦБ_М00.log", "М00", СокрЛП(Агент.Код)));
	Если Файл.Существует() Тогда
		РегистрыСведений.ПоказателиАгентов.ЗаписатьСвойство(Агент.Компьютер,, "КлиентОтчиталсяОПолученииФайлаВыгрузки", Истина);
		Возврат Истина;
	КонецЕсли;
...
Функция ПутьКПлатформе(Агент)
	...
	Если Ложь Тогда Агент = Справочники.ИнформационныеБазы.ПустаяСсылка() КонецЕсли;
	
	Если Агент.Компьютер.Разрядность = "32" Тогда
...
Функция КаталогПлатформы(Агент)
	...
	Если Агент.Разрядность = "32-bit" Тогда
...
Функция Версия1С(Агент)
	...
	Возврат Агент.Компьютер.ВерсияПлатформы;
...
Процедура ПроверитьСостояние(Команда)
	...
	Для каждого ТекущийИндекс Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		КодКоманды = ПутьКПлатформе(Ссылка) + " /F """ + КаталогБазы + """" + ФрагментКомандыУчетныеДанные() + " /DisableStartupMessages /Execute """ + КаталогОбработок() + "GetState.epf"" /C ""ID:" + СтрЗаменить(IP, ".", "_") + ";""";
		ПаузаПослеВыполнения = Элементы.Список.ВыделенныеСтроки.Количество() = 1 И НеЗакрыватьКонсольПослеВыполнения;
		ЗапуститьУдаленно(ip, КодКоманды, , , ПаузаПослеВыполнения);
...
Процедура Запустить1СПодАдмином(Команда)
	...
	Для каждого ТекущийИндекс Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		КодКоманды = ПутьКПлатформе(Ссылка) + " /F """ + КаталогБазы + """" + ФрагментКомандыУчетныеДанные() + "";
		ПаузаПослеВыполнения = Элементы.Список.ВыделенныеСтроки.Количество() = 1 И НеЗакрыватьКонсольПослеВыполнения;
		ЗапуститьУдаленно(ip, КодКоманды, , , ПаузаПослеВыполнения);
...
Процедура ОбновитьКонфигурациюБД(Команда)
	...
	Для каждого ТекущийИндекс Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		|taskkill /IM httpd* /f
		|taskkill /IM 1c* /f
		|" + ПутьКПлатформе(Ссылка)+ " CONFIG /Visible /F """ + КаталогБазы + """" + ФрагментКомандыУчетныеДанные() + " /UpdateDBCfg
		|net start Apache
		|";
		ПаузаПослеВыполнения = Элементы.Список.ВыделенныеСтроки.Количество() = 1 И НеЗакрыватьКонсольПослеВыполнения;
		ЗапуститьУдаленно(ip, КодКоманды, , , ПаузаПослеВыполнения);
...
Процедура ВыполнитьВнешнююОбработкуУдаленно(Знач ИмяФайла, Знач Интерактивно, Знач ПодСистемнойУчетнойЗаписью)
	...
	Для каждого ТекущийИндекс Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		КодКоманды = ПутьКПлатформе(Ссылка) + " /F""" + КаталогБазы + """" + ФрагментКомандыУчетныеДанные() + " /DisableStartupMessages /Execute """ + ИмяФайла + """";
		ПаузаПослеВыполнения = Элементы.Список.ВыделенныеСтроки.Количество() = 1 И НеЗакрыватьКонсольПослеВыполнения;
