...
Функция НачатьУстановку(Знач Компьютер, Знач Роль) Экспорт
...
Функция Установить(Знач Компьютер, Знач Роль) Экспорт
...
Функция НачатьУдаление(Знач Компьютер, Знач Роль) Экспорт
	
	Ключ = ФоновоеВыполнение.КлючЗадания(Компьютер, Роль, "Удаление");
	Если ФоновоеВыполнение.НайтиАктивноеЗаданиеПоКлючу(Ключ) = Неопределено Тогда
		ПараметрыЗадания = Новый Массив;
		ПараметрыЗадания.Добавить(Компьютер);
		ПараметрыЗадания.Добавить(Роль);
		ФоновыеЗадания.Выполнить("ФоновоеВыполнение.УдалитьРоль", ПараметрыЗадания, Ключ, Ключ);
	КонецЕсли;
...
Функция Удалить(Знач Компьютер, Знач Роль) Экспорт
...
Функция ВыполнитьМетод(Знач Компьютер, Знач Роль, ИмяМетода, ПараметрыМетода = Неопределено) Экспорт
	
	Запись = СоздатьЗапись(Компьютер, Роль, ИмяМетода);
	...
	Результат = Проверить(Компьютер, Роль);
	Если НЕ Результат.КодВозврата = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	ЗакрытьЗапись(Запись, Истина, Ложь, "", "");
	Выполнить(СтрШаблон("Результат = МенеджерРоли.%1(Компьютер, ПараметрыМетода)", ИмяМетода));
	Если НЕ Результат.КодВозврата = 0 Тогда
		ЗакрытьЗапись(Запись, Ложь, Истина, Результат.ОписаниеОшибки, Результат.Лог);
		Возврат Результат;
	КонецЕсли;
