...
Функция ВыполнитьPowershell(Знач Компьютер, Знач ИмяФайлаСкрипта, Интерактивно = Ложь, ОтИмениСистемы = Ложь) Экспорт 
	...
	Обработчик = РегистрыСведений.РолиКомпьютеров.Роль("Powershell");
	Возврат Обработчик.ВыполнитьСкрипт(Компьютер, ИмяФайлаСкрипта, Интерактивно, ОтИмениСистемы);

КонецФункции

Функция ВыполнитьCMD(Знач Компьютер, Знач ИмяФайлаСкрипта, Интерактивно = Ложь, ОтИмениСистемы = Ложь) Экспорт 
	...
	ИмяФайлаСтартера = СоздатьСкрипт(СтрШаблон("%1\psexec.exe \\%2 -accepteula %3 %4 -h -u %5 -p %6 -c %7 > %8"
		, КаталогУтилит(), Адрес, ?(Интерактивно, " -i", ""), ?(ОтИмениСистемы, " -s", "")
		, УчетныеДанные.Логин, УчетныеДанные.Пароль, ИмяФайлаСкрипта, ИмяФайлаЛога));
	КодВозврата = 0;
	ЗапуститьПриложение(ИмяФайлаСтартера, КаталогВременныхФайлов(), Истина, КодВозврата);
...
Функция ВыполнитьКомандуCMD(Знач Компьютер, Знач Команда, Интерактивно = Ложь, ОтИмениСистемы = Ложь) Экспорт

	ИмяФайлаСкрипта = СоздатьСкрипт(Команда);
	Результат = ВыполнитьCMD(Компьютер, ИмяФайлаСкрипта, Интерактивно, ОтИмениСистемы);
	ФайловаяСистема.УдалитьФайл(ИмяФайлаСкрипта);
	Возврат Результат;
...
Функция IPCДоступен(Компьютер, ИспользоватьКэш = Истина) Экспорт
	...
	Если ИспользоватьКэш Тогда
		Результат = РегистрыСведений.СвойстваКомпьютеров.ПолучитьСвойство(Компьютер, , "IPC");
	...
	Если Результат.КодВозврата = 0 Тогда
		РегистрыСведений.СвойстваКомпьютеров.ЗаписатьСвойство(Компьютер, , "IPC", 1);
	...
	Иначе
		...
		РегистрыСведений.СвойстваКомпьютеров.ЗаписатьСвойство(Компьютер, , "IPC", 0, Результат.Лог);
		Возврат Результат;
...
Функция ВыполнитьБазовуюНастройку(Компьютер, Принудительно = Ложь) Экспорт
	...
	Если НЕ Принудительно И РегистрыСведений.СвойстваКомпьютеров.ПолучитьСвойство(Компьютер, , "БазоваяНастройкаВыполнена").Значение = "1" Тогда
		Возврат Результат(0);
	КонецЕсли;
	...
	РегистрыСведений.СвойстваКомпьютеров.ЗаписатьСвойство(Компьютер, , "БазоваяНастройкаВыполнена", 1);
	Возврат Результат(2);
...
Функция СкопироватьФайл(Знач Компьютер, Знач Откуда, Знач Куда = Неопределено, Знач Принудительно = Ложь) Экспорт
	...
	УчетныеДанные = УчетныеДанные(Компьютер); 
	ИмяФайлаСкрипта = СоздатьСкрипт(СтрШаблон(Тело, Компьютер.IP, Откуда, СтрЗаменить(Куда, "c:", "c$"), УчетныеДанные.Логин, УчетныеДанные.Пароль));
...
Функция УдалитьФайл(Знач Компьютер, Знач Куда) Экспорт
	...
	УчетныеДанные = УчетныеДанные(Компьютер); 
	ИмяФайлаСкрипта = СоздатьСкрипт(СтрШаблон(Тело, Компьютер.IP, СтрЗаменить(Куда, "c:", "c$"), УчетныеДанные.Логин, УчетныеДанные.Пароль));
	ИмяФайлаЛога = СтрЗаменить(ИмяФайлаСкрипта, ".cmd", ".log");
...
Функция ЗапроситьРазрядностьОС(Компьютер) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат.Разрядность) И НЕ Компьютер.Разрядность = Результат.Разрядность Тогда
		РегистрыСведений.СвойстваКомпьютеров.ЗаписатьСвойство(Компьютер, , "Разраядность", Результат.Лог); 
		Возврат Результат(0);
	КонецЕсли;
