...
Функция ПолучитьПроксиПоУмолчанию(URL)
	...
	Если МодульПолучениеФайловИзИнтернетаКлиентСервер <> Неопределено Тогда
		СтруктураURL = РазобратьURL(URL);
		ПроксиПоУмолчанию = МодульПолучениеФайловИзИнтернетаКлиентСервер.ПолучитьПрокси(СтруктураURL.Схема);
	КонецЕсли;
...
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПараметрыПреобразования)
	...
	Исключение
		Объект = ПолучитьСтрокуИзДвоичныхДанных (Json);
...
Функция РаспаковатьОтвет(Ответ)
	...
	Если Заголовок <> Ложь Тогда
		...
		ИначеЕсли НРег(Заголовок) = "1cstor" Тогда
		    СтрокаBase64 = Base64Строка(Ответ.Тело);
		    Хранилище = СериализаторXDTO.XMLЗначение(Тип("ХранилищеЗначения"), СтрокаBase64);
...
Функция ЗаголовкиПоУмолчанию()
	...
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Accept-Encoding", "gzip,1cstor");
