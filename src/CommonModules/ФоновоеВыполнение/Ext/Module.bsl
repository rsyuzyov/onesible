...
Процедура ВыполнениеОбменов() Экспорт
	...
	Пока ВыборкаПланы.Следующий() Цикл
		...
		КонецЕсли;
		АктивныхЗаданий = Цел(ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("Метод, Состояние", "Фастекс.ВыполнитьОбмен", СостояниеФоновогоЗадания.Активно)).Количество() / 2);
		...
		Пока Выборка.Следующий() Цикл
			...
			Ключ = "" + Выборка.Узел.Код + "." + "Выгрузка";
			Если НайтиАктивноеЗаданиеПоКлючу(Ключ) = Неопределено Тогда
			...
			Ключ = "" + Выборка.Узел.Код + "." + "Загрузка";
			Если НайтиАктивноеЗаданиеПоКлючу(Ключ) = Неопределено Тогда
				ПараметрыЗадания = Новый Массив;
				ПараметрыЗадания.Добавить(Выборка.ПланОбмена);
				ПараметрыЗадания.Добавить(Выборка.Узел);
